...
# Ensure ischemic_stroke_30 is numeric
syn_data <- syn_data %>%
  mutate(ischemic_stroke_30 = as.numeric(as.character(ischemic_stroke_30)))

# Calculate reference timing for each study (T_i0)
ref_timing <- syn_data %>%
  group_by(trial) %>%
  dplyr::summarize(ref_timing = min(timing_DOAC, na.rm = TRUE))
...
plotdata_studies <- lapply(studies, function(study_i) {
  ...
  if (length(ref_i) > 1) {
   ...
   mod <- glm(
    formula = ischemic_stroke_30 ~ I(adjusted_timing),
    data    = df_sub,
    family  = binomial
  )
  ...
  do({
    ...
    .x %>%
      ...
      dplyr::summarize(
        ...
        obs_risk   = mean(ischemic_stroke_30, na.rm = TRUE),
        .groups    = "drop"
      )
  }) %>%
  ungroup()
...
Model3_RIS_separate_plot <- ggplot() +
...
Model3_RIS_separate_plot


Model3_RIS_separate_plot_logy <- Model3_RIS_separate_plot +
  scale_y_log10() 
...
jags_data <- list(
  ...
  ischemic_stroke_30 = syn_data$ischemic_stroke_30, # Outcome variable
...
Model3_RIS_Linear <- jags.parallel(data = jags_data,inits=NULL,parameters.to.save = c("B", "tau","tau2", "u", "beta"),model.file = modelLinearTiming_3,
                               n.chains=2,n.iter = 20000,n.burnin = 2000,DIC=F,n.thin = 10)
print(Model3_RIS_Linear)

#### Tables of results
mcmc_sum <- Model3_RIS_Linear$BUGSoutput$summary
...
# The summary of JAGS model output:
# Model3_RIS_Linear$BUGSoutput$summary is a matrix with rows like:
#  "B", "tau", "u[1]", "u[2]", ..., "beta[1]", "beta[2]", ...

# Posterior means for study-specific intercepts and slopes
u_summaries <- Model3_RIS_Linear$BUGSoutput$summary[
  grep("^u\\[", rownames(Model3_RIS_Linear$BUGSoutput$summary)),
  "mean"
]
beta_summaries <- Model3_RIS_Linear$BUGSoutput$summary[
  grep("^beta\\[", rownames(Model3_RIS_Linear$BUGSoutput$summary)),
  "mean"
]
...
B_mean <- Model3_RIS_Linear$BUGSoutput$summary["B","mean"]
...
Model3_RIS_plot_log <- Model3_RIS_plot + scale_y_log10(breaks = c(0.001, 0.01, 0.03, 0.05, 0.07, 0.1))

Model3_RIS_plot_log


save(Model3_RIS_Linear, Model3_RIS_plot_log, Model3_RIS_plot, Model3_RIS_separate_plot, Model3_RIS_separate_plot_logy, file="Saved_Data/Model3_RIS_Linear.RData")
