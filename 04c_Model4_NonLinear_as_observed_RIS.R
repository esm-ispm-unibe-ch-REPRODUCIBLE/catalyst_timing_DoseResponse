...
ref_timing <- syn_data %>%
  group_by(trial) %>%
  dplyr::summarize(ref_timing =min(timing_DOAC[ischemic_stroke_30 == 0], na.rm = TRUE))
...
fit_glm_rcs_ELANmod <- glm(
  ischemic_stroke_30 ~ rcs(adjusted_timing, my_knots_ELANmod, 3),
...
fit_glm_rcs_ELANmin <- glm(
  ischemic_stroke_30 ~ rcs(adjusted_timing, my_knots_ELANmin, 3),
...
fit_glm_rcs_ELANmaj <- glm(
  ischemic_stroke_30 ~ rcs(adjusted_timing, my_knots_ELANmaj, 3),
  data   = syn_data_ELANmaj,
  family = binomial()
)
summary(fit_glm_rcs_ELANmaj)
fit_glm_ELANmaj <- glm(
  ischemic_stroke_30 ~ adjusted_timing,
  data   = syn_data_ELANmaj,
  family = binomial()
)
...
fit_glm_rcs_TIMING<- glm(
  ischemic_stroke_30 ~ rcs(adjusted_timing, my_knots_TIMING, 3),
...
fit_glm_rcs_OPTIMAS <- glm(
  ischemic_stroke_30 ~ rcs(adjusted_timing, my_knots_OPTIMAS, 3),
...
fit_glm_rcs_START <- glm(
  ischemic_stroke_30 ~ rcs(adjusted_timing, my_knots_START, 3),
  data   = syn_data_START,
  family = binomial()
)
...
Model5_RIS_IPD_seperate <- ggplot(df_all_glm2, 
                              aes(x = timing_DOAC, 
...
print(Model4_RIS_RCS)


# Extract the MCMC summary from the MVN model
mcmc_sum <- Model4_RIS_RCS$BUGSoutput$summary
...
Model4_RIS_RCS_plot_log <- Model4_RIS_RCS_plot +  scale_y_log10(breaks = c(0.001, 0.01, 0.03, 0.05, 0.07, 0.1))

Model4_RIS_RCS_plot_log


save(Model5_RIS_IPD_seperate,Model4_RIS_RCS,Model4_RIS_RCS_plot_log, Model4_RIS_RCS_plot, file="Saved_Data/Model4_RIC_RCS.RData")
