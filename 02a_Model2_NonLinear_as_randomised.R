...
knots <- quantile(syn_data$avg_rando_timing_DOAC, probs = c(0.1, 0.5, 0.9), na.rm = TRUE)
rcs_basis <- rcspline.eval(syn_data$avg_rando_timing_DOAC, knots = knots, inclx = TRUE)
...
# -- JAGS model with random effects -- #
...
mcmc_sum <- Model2_MVN$BUGSoutput$summary
...
# Study-specific spline coefficients: Beta[i, 1], Beta[i, 2]
...
# Identify your study IDs in the model's order
...
ModelRCS2_plot_log <- ModelRCS2_plot +  scale_y_log10(breaks = c(0.001, 0.01, 0.03, 0.05, 0.07, 0.1))

ModelRCS2_plot_log

save(Model2_MVN, ModelRCS2_plot_log, ModelRCS2_plot_log, file="Saved_Data/Model2_rcs.RData")
