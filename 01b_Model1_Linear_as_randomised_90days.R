...
ref_timing <- syn_data %>%
  group_by(trial) %>%
  dplyr::summarize(ref_timing = min(avg_rando_timing_DOAC[composite_90 == 0], na.rm = TRUE))
...
plotdata_studies <- lapply(studies, function(study_i) {
  ...
  # We'll do: composite_90 ~ I(avg_rando_timing_DOAC - ref_i)
  # so the intercept is the risk at reference, slope is the effect of timing
  mod <- glm(
    formula = composite_90 ~ I(avg_rando_timing_DOAC - ref_i),
    data    = df_sub,
    family  = binomial
  )
...
observed_means <- syn_data %>%
  ...
  dplyr::summarize(
    obs_mean  = mean(composite_90, na.rm = TRUE),
    obs_count = n(),
    .groups   = "drop"
  )
...
Model1_90_separate_plot <- ggplot() +
...
Model1_90_separate_plot


Model1_90_separate_plot_logy <- Model1_90_separate_plot +
  scale_y_log10() 
...
Model1_90 <- jags.parallel(data = jags_data,inits=NULL,parameters.to.save = c("B", "tau","tau2", "u", "beta"),model.file = modelLinearTiming,
                        n.chains=2,n.iter = 20000,n.burnin = 2000,DIC=F,n.thin = 10)
print(Model1_90, digits = 2)


#traceplot(as.mcmc(Model1))

#### Tables of results
mcmc_sum <- Model1_90$BUGSoutput$summary
...
# ------------------------------------------------------------------
# 1) Posterior means for each study's intercept and slope
#    (for the study-specific lines)
# ------------------------------------------------------------------
...
Model1_90_plot_log <- Model1_90_plot + scale_y_log10(breaks = c(0.001, 0.01, 0.03, 0.05, 0.07, 0.1))

Model1_90_plot_log



save(Model1_90, Model1_90_plot_log, Model1_90_plot, Model1_90_separate_plot, Model1_90_separate_plot_logy, file="Saved_Data/Model1_90_Linear.RData")
