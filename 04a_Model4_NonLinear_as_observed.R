...
fit_glm_rcs_ELANmod <- glm(
  composite_30 ~ rcs(adjusted_timing, my_knots_ELANmod, 3),
...
fit_glm_rcs_ELANmin <- glm(
  composite_30 ~ rcs(adjusted_timing, my_knots_ELANmin, 3),
...
fit_glm_rcs_ELANmaj <- glm(
  composite_30 ~ rcs(adjusted_timing, my_knots_ELANmaj, 3),
  data   = syn_data_ELANmaj,
  family = binomial()
)
summary(fit_glm_rcs_ELANmaj)
fit_glm_ELANmaj <- glm(
  composite_30 ~ adjusted_timing,
  data   = syn_data_ELANmaj,
  family = binomial()
)
...
fit_glm_rcs_TIMING<- glm(
  composite_30 ~ rcs(adjusted_timing, my_knots_TIMING, 3),
...
fit_glm_rcs_OPTIMAS <- glm(
  composite_30 ~ rcs(adjusted_timing, my_knots_OPTIMAS, 3),
...
fit_glm_rcs_START <- glm(
  composite_30 ~ rcs(adjusted_timing, my_knots_START, 3),
  data   = syn_data_START,
  family = binomial()
)
...
Model5_IPD_seperate <- ggplot(df_all_glm2, 
                     aes(x = timing_DOAC, 
  ...
  # 3) Control the range of line widths
...
mcmc_sum <- Model4_RCS$BUGSoutput$summary
...
########################################
#  Define knots & build 2-column RCS
########################################
...
########################################
# Extract posterior means for each study
########################################
...
########################################
# Lines from the model for each study
########################################
...
########################################
# 95% CI for that pooled line (iteration-wise)
########################################
...
Model4_RCS_plot_log <- Model4_RCS_plot +  scale_y_log10(breaks = c(0.001, 0.01, 0.03, 0.05, 0.07, 0.1))

Model4_RCS_plot_log

save(Model5_IPD_seperate, Model4_RCS, Model4_RCS_plot_log, Model4_RCS_plot, file="Saved_Data/Model4_RCS.RData")
