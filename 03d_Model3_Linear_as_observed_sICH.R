...
plotdata_studies <- lapply(studies, function(study_i) {
  ...
  if (length(ref_i) > 1) {
   ...
   mod <- logistf(
    formula = SICH_stroke_30 ~ I(adjusted_timing),
    data    = df_sub,
    family  = binomial
  )
...
Model3_SICH_separate_plot <- ggplot() +
...
Model3_SICH_separate_plot + scale_size_continuous(range = c(0.5, 3), guide = "none")



Model3_SICH_separate_plot_logy <- Model3_SICH_separate_plot +
  scale_y_log10() + scale_size_continuous(range = c(0.5, 3), guide = "none")
...
jags_data <- list(
  ...
  SICH_stroke_30 = syn_data$SICH_stroke_30, # Outcome variable
...
Model3_SICH <- jags.parallel(data = jags_data,inits=NULL,parameters.to.save = c("B", "tau", "tau2","u", "beta"),model.file = modelLinearTiming_3,
                                   n.chains=2,n.iter = 50000,n.burnin = 2000,DIC=F,n.thin = 10)
print(Model3_SICH)

#### Tables of results
mcmc_sum <- Model3_SICH$BUGSoutput$summary
...
################### PLOTS ################################
u_summaries <- Model3_SICH$BUGSoutput$summary[
  grep("^u\\[", rownames(Model3_SICH$BUGSoutput$summary)),
  "mean"
]
beta_summaries <- Model3_SICH$BUGSoutput$summary[
  grep("^beta\\[", rownames(Model3_SICH$BUGSoutput$summary)),
  "mean"
]
...
B_mean <- Model3_SICH$BUGSoutput$summary["B","mean"]
...
logit_line <- 4+ alpha_mean + B_mean * (x_seq_pooled - 0)
...
Model3_SICH_plot_log <- Model3_SICH_plot + scale_y_log10(breaks = c(0.001, 0.01, 0.03, 0.05, 0.07, 0.1))

Model3_SICH_plot_log

save(Model3_SICH, Model3_SICH_plot_log, Model3_SICH_plot, Model3_SICH_separate_plot, Model3_SICH_separate_plot_logy, file="Saved_Data/Model3_SICH.RData")
